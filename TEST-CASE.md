...
| 目标 URL | `https://labradorsedsota.github.io/online-exam-system-singer/` |
...
```bash
# 1. 打开目标页面
open -a "Google Chrome" "https://labradorsedsota.github.io/online-exam-system-singer/"
sleep 3
...
2. 结果页顶部成绩概览区域,得分数字显示为绿色(及格色,不应为红色),因为张三 92 分超过及格线 60 分
3. 第1题(单选题,10分)显示得分"10 / 10 分"
4. 选项B标记为正确答案(✓标记)
5. 考生选择B,与正确答案匹配,显示正确标记
...
| 关联 AC | AC-F05-02 |
| 设计技术 | 触发路径补充(_autoGrade 实时调用) |
| 数据依赖 | CUSTOM - 需要含多选题的 in-progress 记录 |
| 冲突标记 | 无 |
| 前置策略 | 策略一(完全清空 + 自定义注入) |
| fixture | 无(内联注入) |
...
```javascript
// Step 1: 清空
localStorage.removeItem('oes_questions');
localStorage.removeItem('oes_papers');
localStorage.removeItem('oes_records');

// Step 2: 注入两道多选题
const questions = [
  { id: 'q_m1', type: 'multiple', stem: '以下哪些是前端框架?', options: ['React', 'Django', 'Vue', 'Flask'], correctAnswers: [0, 2], createdAt: Date.now() },
  { id: 'q_m2', type: 'multiple', stem: '以下哪些是编程语言?', options: ['JavaScript', 'HTML', 'Python', 'CSS'], correctAnswers: [0, 2], createdAt: Date.now() }
];
localStorage.setItem('oes_questions', JSON.stringify(questions));

// Step 3: 注入已发布试卷
const papers = [{
  id: 'p_mc', name: '多选专项测试', status: 'published', switchOn: true,
  duration: 60, passScore: 10, description: '', startTime: null, endTime: null,
  questions: [
    { questionId: 'q_m1', score: 15 },
    { questionId: 'q_m2', score: 10 }
  ],
  createdAt: Date.now()
}];
localStorage.setItem('oes_papers', JSON.stringify(papers));

// Step 4: 注入 in-progress 记录(多选题全部答对)
const records = [{
  id: 'r_mc1', paperId: 'p_mc', studentName: '测试生',
  answers: { q_m1: [0, 2], q_m2: [0, 2] },
  flagged: {}, status: 'in-progress',
  startTime: Date.now() - 600000, submitTime: null,
  scores: {}, totalScore: 0, createdAt: Date.now()
}];
localStorage.setItem('oes_records', JSON.stringify(records));
```

导航至 `/#/student/exam/p_mc`

**任务描述:**
当前页面是答题页,显示"多选专项测试"试卷。题目已经作答完毕。请点击页面右上角的"提交试卷"按钮,在弹出的确认对话框中点击"确认提交"。提交后页面会跳转到考试结果页。在结果页上,查看第1题(多选题,"以下哪些是前端框架?",15分)的得分显示。仅在当前页面操作,不要导航到其他网址。

**Expected Results(逐条):**
1. 点击提交后,确认对话框出现
2. 点击确认提交后,页面跳转到考试结果页
3. 第1题多选题("以下哪些是前端框架?",15分)显示得分"15 / 15 分"(答案全对应得满分)
...
**任务描述:**
在当前页面(题库管理页),先确认当前"全部"Tab高亮且列表显示8道题目。然后点击题型筛选Tab"多选",等待1-2秒后观察列表变化--重点检查列表中的题目数量是否从8道减少为2道。如果点击"多选"Tab后列表仍然显示8道题目没有变化,说明筛选功能失效,应判定为 FAIL。再点击"全部"Tab恢复。仅在当前页面操作,不要导航到其他网址。
